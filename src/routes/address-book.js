...
const list3 = async (req)=>{
    ...
    [rows] = await db.query("select * from members t1 inner join bidding_chang t2 on t2.sid=t1.sid inner join product_chang t3 on t3.product_id=t2.product_id ORDER BY bid_created_time DESC");
    rows.forEach((row) => {
